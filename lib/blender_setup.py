...
def parseOptions(argv=None):
    ...
    # Misc
    op.add_option('--use-stamp', dest='use_stamp', \
...
def mk_tmp_dir(tmp_dir):
    ...
    try:
        ...
        try:
            ...
            raise Exception('Unable to set up temporary directory: ' +
    ...
    if stat.S_ISDIR(mode):
        try:
            ...
            raise Exception('Unable to set up temporary directory: ' +
                            'unable to clean existing temporary directory')
        ...
        raise Exception('Unable to set up temporary directory: ' +
                '{0} is neither directory nor regular file' . format(tmp_dir))
...
def setupScenes(opts):
    ...
    for scene in bpy.data.scenes:
        ...
        if rd.parts_x < int(opts.tiles_x):
            rd.parts_x = int(opts.tiles_x)

        if rd.parts_y < int(opts.tiles_y):
            rd.parts_y = int(opts.tiles_y)
...
def main():
    ...
    if opts.node_id is None or opts.output_path is None:
        raise Exception('Missed at least one of mandatory arguments: ' +
                        '--node-id or --output-path')
